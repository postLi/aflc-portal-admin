    ...
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="250px" class="demo-ruleForm">
      ...
      <div class="searchInformation information">
        ...
        <el-form-item label="出发地：" prop="startLocation">
          ...
          <vregion :ui="true" @values="regionChangeStart" :ifAera='true' class="form-control"
                   @testCity="ifProvice('startLocation')" v-else>
        ...
        <el-form-item label="联系电话：" prop="startLocationContactsMobile" label-width="150px">
          <el-input v-model="ruleForm.startLocationContactsMobile" :disabled="unable" v-numberOnly
      ...
      <div class="information priceTime">
        ...
        <el-form-item label="重货价格：" prop="weigthPriceForms">
          ...
          <div class="goodsPriceChoose">
            ...
            <ul v-for="(form,keys) in weigthPriceForms" :key="keys">
              <li>
                <el-input v-model.number="form.startVolume" v-numberOnly placeholder="包含" maxlength="7"
                          disabled></el-input>
                <span>----</span>
                ...
                <input class="nativeinput" :value="form.endVolume"
                       @change="(e)=>{setInputVal(e.target.value,form, 'endVolume'),ifWrong(weigthPriceForms,keys)}"
                       :maxlength="7" placeholder="不包含" auto-complete="off" clearable
              ...
              <li class="buttons">
                            <span @click="addItem('weight',keys,form)" class="addItem"
                                  v-if="keys == weigthPriceForms.length-1 && keys != 4">
                            </span>
                <span @click="reduceItem(keys,'weight')" class="reduceItem"
                      v-if="keys == weigthPriceForms.length-1 && weigthPriceForms.length !=1 ">
                            </span>
              </li>
            </ul>
          </div>
          ...
          <div class="goodsPriceChoose">
            ...
            <ul v-for="(form,keys) in ligthPriceForms" :key="keys">
              <li>
                ...
                <input class="nativeinput" :value="form.endVolume"
                       @change="(e)=>{setInputVal(e.target.value,form, 'endVolume'),ifWrong(ligthPriceForms,keys)}"
                       :maxlength="7" placeholder="不包含" auto-complete="off" clearable
                       v-number-only :disabled="unable" type="text">
                立方
              </li>
              ...
              <li class="buttons">
                            <span @click="addItem('light',keys,form)" class="addItem"
                                  v-if="keys == ligthPriceForms.length-1 && keys != 4">
                            </span>
                <span @click="reduceItem(keys,'light')" class="reduceItem"
                      v-if="keys == ligthPriceForms.length-1 && ligthPriceForms.length !=1">
                            </span>
              </li>
            </ul>
          </div>
        ...
        <el-form-item label="最低一票价格：" prop="lowerPrice">
          <el-input v-model="ruleForm.lowerPrice" placeholder="报价" :disabled="unable" v-number-only:point></el-input>
          元
        </el-form-item>
        ...
        <el-form-item label="专线照片：" prop="rangeLogo">
          <upload class="licensePicture" v-if=" !unable" tip="（必须为jpg/png并且小于5M）" :disabled="unable" :limit="3"
                  listtype="picture-card" :showFileList='true' v-model="ruleForm.rangeLogo"/>
